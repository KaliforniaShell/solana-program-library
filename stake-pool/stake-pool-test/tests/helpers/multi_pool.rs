...
pub struct MultiPoolValidatorStakeAccount {
...
impl MultiPoolValidatorStakeAccount {
    pub fn new(
        stake_pool: &Pubkey,
        ...
        MultiPoolValidatorStakeAccount {
            stake_account,
...
pub async fn simple_add_validator_to_pool(
    ...
    payer: &Keypair,
    recent_blockhash: &Hash,
    stake_pool_accounts: &MultiPoolAccounts,
    sol_deposit_authority: Option<&Keypair>,
) -> MultiPoolValidatorStakeAccount {
    let validator_stake = MultiPoolValidatorStakeAccount::new(
...
#[derive(Debug)]
pub struct MultiPoolDepositStakeAccount {
...
impl MultiPoolDepositStakeAccount {
...
pub async fn simple_deposit_stake(
    ...
    recent_blockhash: &Hash,
    stake_pool_accounts: &MultiPoolAccounts,
    validator_stake_account: &MultiPoolValidatorStakeAccount,
    stake_lamports: u64,
) -> Option<MultiPoolDepositStakeAccount> {
    let authority = Keypair::new();
    // make stake account
    let stake = Keypair::new();
    let lockup = stake::state::Lockup::default();
    ...
    Some(MultiPoolDepositStakeAccount {
        authority,
        stake,
        pool_account,
        stake_lamports,
...
/* XXX HANA might need refactoring to handle this
pub async fn setup_for_withdraw(
    ...
    MultiPoolAccounts,
    MultiPoolValidatorStakeAccount,
    MultiPoolDepositStakeAccount,
    Keypair,
    Keypair,
    u64,
) {
    let mut context = program_test().start_with_context().await;
    let stake_pool_accounts = MultiPoolAccounts::new_with_token_program(token_program_id);
    stake_pool_accounts
